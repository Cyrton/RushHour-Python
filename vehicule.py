        ...
        for x in data_dict:
            if x == pseudo:
                for y in data_dict[x]["score"]:
                    if self.niveau == 1:
                        if data_dict[x]["score"][0] < score:
                            data_dict[x]["score"][0] = score
                            with open('sauvegarde.json','w') as json_data_w:
                                json.dump(data_dict,json_data_w)
                        break
                    if self.niveau == 2:
                        if data_dict[x]["score"][1] < score:
                            data_dict[x]["score"][1] = score
                            with open('sauvegarde.json','w') as json_data_w:
                                json.dump(data_dict,json_data_w)
                        break
                    if self.niveau == 3:
                        if data_dict[x]["score"][2] < score:
                            data_dict[x]["score"][2] = score
                            with open('sauvegarde.json','w') as json_data_w:
                                json.dump(data_dict,json_data_w)
                        break
